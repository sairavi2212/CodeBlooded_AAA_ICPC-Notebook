...
/*
  ...
  - Methods:
      ...
      void range_add(int l, int r, T v): add v to every element in [l, r] (0-based inclusive)
      T prefix_sum(int idx)            : sum of elements [0..idx] (0-based). idx < 0 -> 0
      T range_sum(int l, int r)        : sum of elements [l..r] (0-based)
      T point_query(int idx)           : value at index idx (0-based)
*/
template<typename T = long long>
class FenwickRange {
public:
    explicit FenwickRange(int n = 0) : n_(n), B1(n_ + 1, T(0)), B2(n_ + 1, T(0)) {}
...
private:
    ...
    static void add(vector<T>& bit, int idx1, T val) {
        ...
        for (int i = idx1; i <= n; i += (i & -i)) bit[i] += val;
    }

    static T sum(const vector<T>& bit, int idx1) {
        if (idx1 <= 0) return T(0);
        int i = idx1;
        T res = T(0);
