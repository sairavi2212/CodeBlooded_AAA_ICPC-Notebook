...
template<typename T>
using ordered_set = tree<pair<T,int>, null_type, less<pair<T,int>>, rb_tree_tag, tree_order_statistics_node_update>;
...
class MergeSortTree {
    ...
    void insert_at(int node, int l, int r, int pos, int val) {
        ...
        if (l == r) return;
        int mid = (l + r) >> 1;
        if (pos <= mid) insert_at(node<<1, l, mid, pos, val);
        else insert_at(node<<1|1, mid+1, r, pos, val);
    }
    ...
    void erase_at(int node, int l, int r, int pos, int val) {
        ...
        if (l == r) return;
        int mid = (l + r) >> 1;
        if (pos <= mid) erase_at(node<<1, l, mid, pos, val);
        else erase_at(node<<1|1, mid+1, r, pos, val);
    }
    ...
    int query_le_rec(int node, int l, int r, int ql, int qr, int k) {
        ...
        int mid = (l + r) >> 1;
        return query_le_rec(node<<1, l, mid, ql, qr, k) + query_le_rec(node<<1|1, mid+1, r, ql, qr, k);
    }
...
public:
    ...
    void build(const vector<int> &a) {
        ...
        for (int i = 0; i < n; ++i) insert_at(1, 0, n-1, i, arr[i]);
    }
    ...
    int count_lt(int l, int r, int k) {
        return count_le(l, r, k-1);
    }

    int count_gt(int l, int r, int k) {
        return (r - l + 1) - count_le(l, r, k);
    }
