...
struct LCP {
    ...
    void build(const string& str) {
        ...
        n = (int)s.size();
        if (n == 0) {
            sa.clear(); lcp.clear(); rank.clear(); st.clear();
            return;
        }
    ...
    // answer LCP of suffixes starting at i and j (0-indexed positions in original string)
    int query(int i, int j) const {
        if (i < 0 || j < 0 || i >= n || j >= n) return 0; // invalid -> 0
        if (i == j) return n - i; // full suffix matches itself
    ...
    // ---------------------------
    // Explanation:
    // For a string of length n, total number of substrings (including duplicates) is:
    //   total = n * (n + 1) / 2
    //
